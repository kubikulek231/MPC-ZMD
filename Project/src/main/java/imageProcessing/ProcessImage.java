...
public class ProcessImage {
    ...
    public void convertToYCbCr() {
        ...
        for (int x = 0; x < imageWidth; x++) {
            for (int y = 0; y < imageHeight; y++) {
                double r = originalState.getRgb().getRed()[x][y];
                double g = originalState.getRgb().getGreen()[x][y];
                double b = originalState.getRgb().getBlue()[x][y];
    ...
    public void convertToRGB() {
        ...
        for (int x = 0; x < imageWidth; x++) {
            for (int y = 0; y < imageHeight; y++) {
                ...
                workingState.getRgb().getRed()[x][y] = clamp((int) Math.round(Y + 1.402  * Cr));
                workingState.getRgb().getGreen()[x][y] = clamp((int) Math.round(Y - 0.344  * Cb - 0.714 * Cr));
                workingState.getRgb().getBlue()[x][y] = clamp((int) Math.round(Y + 1.772  * Cb));
            }
        }
    }
    ...
    public double[] calculateMetrics(QualityType type) {
        switch (type) {
            case RED:   return metricsForChannels(Quality.convertIntToDouble(originalState.getRgb().getRed()),   Quality.convertIntToDouble(workingState.getRgb().getRed()));
            case GREEN: return metricsForChannels(Quality.convertIntToDouble(originalState.getRgb().getGreen()), Quality.convertIntToDouble(workingState.getRgb().getGreen()));
            case BLUE:  return metricsForChannels(Quality.convertIntToDouble(originalState.getRgb().getBlue()),  Quality.convertIntToDouble(workingState.getRgb().getBlue()));
            case Y:     return metricsForChannels(originalState.getYCbCr().getY().getArray(), workingState.getYCbCr().getY().getArray());
            case CB:    return metricsForMatchedChannels(originalState.getYCbCr().getCb(), workingState.getYCbCr().getCb(), "Cb");
            case CR:    return metricsForMatchedChannels(originalState.getYCbCr().getCr(), workingState.getYCbCr().getCr(), "Cr");
            case RGB: {
                double[] r = metricsForChannels(Quality.convertIntToDouble(originalState.getRgb().getRed()),   Quality.convertIntToDouble(workingState.getRgb().getRed()));
                double[] g = metricsForChannels(Quality.convertIntToDouble(originalState.getRgb().getGreen()), Quality.convertIntToDouble(workingState.getRgb().getGreen()));
                double[] b = metricsForChannels(Quality.convertIntToDouble(originalState.getRgb().getBlue()),  Quality.convertIntToDouble(workingState.getRgb().getBlue()));
    ...
    public double[] calculateSSIMMetrics(QualityType type) {
        switch (type) {
            case Y:  return ssimForChannels(originalState.getYCbCr().getY().getArray(),  workingState.getYCbCr().getY().getArray());
            case CB: return ssimForMatchedChannels(originalState.getYCbCr().getCb(), workingState.getYCbCr().getCb(), "Cb");
            case CR: return ssimForMatchedChannels(originalState.getYCbCr().getCr(), workingState.getYCbCr().getCr(), "Cr");
    ...
    public BufferedImage getImageFromRGB() {
        ...
        for (int x = 0; x < imageWidth; x++) {
            for (int y = 0; y < imageHeight; y++) {
                int rgb = (workingState.getRgb().getRed()[x][y] << 16)
                        | (workingState.getRgb().getGreen()[x][y] << 8)
                        | workingState.getRgb().getBlue()[x][y];
